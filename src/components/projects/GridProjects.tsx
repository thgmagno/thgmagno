        ...
        .map((item) => (
          ...
          >
            ...
            <div className="relative z-10 mb-8 mt-5 max-h-20 pl-2">
              <div className="mb-2 flex flex-wrap-reverse justify-between">
                <b>{item.data.title}</b>
                <StatusProject done={item.done} />
              </div>
              <p className="truncate pr-3 text-justify text-sm font-light sm:overflow-visible sm:whitespace-normal">
                {item.data.description[language].slice(0, 120).concat('...')}
              </p>
            </div>
            <div className="relative z-10 mx-2 flex justify-between md:mt-6">
              <div className="no-scrollbar flex max-w-[60%] gap-1 overflow-x-scroll md:max-w-[70%]">
                {item.data.technologies
                  ...
                  .map((tech) => (
                    <Badge
                      key={tech.title}
                      className="min-w-fit bg-neutral-700 dark:bg-neutral-900 dark:text-white dark:hover:bg-neutral-950"
                    >
              ...
              <Link
                ...
                className="flex min-w-fit items-center gap-1 rounded-md border bg-neutral-600 px-2 py-1 text-sm text-neutral-100 hover:bg-neutral-700 active:scale-95 dark:hover:bg-neutral-700"
              >
                {dict[language].label}
              </Link>
            </div>
